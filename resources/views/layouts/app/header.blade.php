...
@endpush 
  ...
  <nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <ul class="navbar-nav ml-auto">
      <!-- Impersonate session alert -->
      @if(app('impersonate')->isImpersonating())
      <li class="nav-item">
        <span class="nav-link blinking">{{ __('Impersonating') }}... </span>
      </li>
      @endif
      ...
      <!-- User Dropdown Menu -->
        <li class="nav-item dropdown user-menu">
            ...
            <div class="dropdown-menu dropdown-menu-right" style="max-width: 180px" aria-labelledby="navbarDropdown">  
                ...
                {{-- <a class="dropdown-item" href="{{ Auth::user()->staff()->exists() ? route('sys.user.settings', Auth::user()->id) }}"><i class="fa fa-user-cog"></i> Settings</a> --}}
                @endpermission
                <a class="dropdown-item" href="{{ route('reset-password') }}"><i class="fas fa-key"></i> Change Password</a>
                <div class="dropdown-divider"></div> 
                @env('local')
                @if(auth()->user()->hasPermission('utility-impersonate') && !app('impersonate')->isImpersonating())         
                <a class="dropdown-item" href="{{ route('impersonate') }}"><i class="fa fa-user-secret"></i> {{ __('Impersonate') }}</a>
                @endif
                @endenv
                {{-- <a class="dropdown-item" href="#"><i class="fa fa-user-lock"></i> Lock</a> --}}
                @if(app('impersonate')->isImpersonating())
                <a class="dropdown-item" href="{{ route('impersonate.leave') }}"><i class="fa fa-user-secret"></i> {{ __('Leave Impersonate') }}</a>
                @else
